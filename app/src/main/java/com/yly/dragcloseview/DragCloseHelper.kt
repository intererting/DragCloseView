...
class DragCloseHelper(private val mContext: Context) : LifecycleObserver {

    companion object {
        // 动画执行时间
        private const val DURATION: Long = 80
            ...
            when (event.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    if (isSwipingToClose ||
                        ...
                        parentV.background.mutate().alpha = (percent * 255 / 1.1).toInt()
    ...
    private fun resetCallBackAnimation() {
        ...
        animatorY?.cancel()
        animatorY = ValueAnimator.ofFloat(mCurrentTranslationY, 0F)?.apply {
